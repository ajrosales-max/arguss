...
def test_scan_results_page_renders_selection_checkboxes(client: TestClient) -> None:
    ...
    with mock.patch.object(dashboard_mod, "get_cached_scan_response", return_value=scan):
        response = client.get("/results/selection-demo")
...
def test_scan_results_auto_merge_checked_by_default(client: TestClient) -> None:
    scan = _cached_scan_dict(
        entries=[
    ...
    with mock.patch.object(dashboard_mod, "get_cached_scan_response", return_value=scan):
        response = client.get("/results/auto-merge-default")

    text = response.text
    auto_idx = text.index('value="cand-safe-pkg-001"')
    review_idx = text.index('value="cand-review-pkg-001"')
    assert "checked" in text[auto_idx : auto_idx + 120]
    assert "checked" not in text[review_idx : review_idx + 120]


def test_scan_results_review_required_unchecked_by_default(client: TestClient) -> None:
    scan = _cached_scan_dict(
        entries=[_cached_entry(package="needs-review", tier="review_required")]
    )
    with mock.patch.object(dashboard_mod, "get_cached_scan_response", return_value=scan):
        response = client.get("/results/review-default")

    idx = response.text.index('value="cand-needs-review-001"')
    assert "checked" not in response.text[idx : idx + 120]


def test_scan_results_action_button_disabled(client: TestClient) -> None:
    scan = _cached_scan_dict(entries=[_cached_entry(package="left-pad", tier="auto_merge")])
    with mock.patch.object(dashboard_mod, "get_cached_scan_response", return_value=scan):
        response = client.get("/results/disabled-action")

    assert 'id="action-selected-btn"' in response.text
    assert "disabled" in response.text.split('id="action-selected-btn"')[1].split(">")[0]
